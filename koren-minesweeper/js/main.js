...
var gStorage = [
  {
    ...
    name: " puki", BestScore: gScore.innerHTML, bestTime: "0000", level: gLevel
  }
];
...
function validateWin(board) {
  ...
  for (var i = 0; i < gBombsLocation.length; i++) {
    ...
    if (!elCell.classList.contains('marked') && elCell.classList.contains("reveald")) return false
  }
  ...
  if (currCell.firstElementChild.innerText === BOMB) {
    ...
    if (gLifeCount > 0) {
      ...
      setTimeout(() => {
        ...
        gBestTime.sort(function (a, b) { return a - b });
        userScoreStorage = { userId: gUserStorageId, name: userName, BestScore: gScore, bestTime: gBestTime[0], level: gLevel }
...
function checkIfInBoard(board, idxI, idxJ) {
  return (idxI >= 0 && idxI < board.length && idxJ >= 0 && idxJ < board[idxI].length);
}
